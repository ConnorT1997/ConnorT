# Investment & Analytics Portfolio

This repository curates selected projects in investment research, portfolio analytics, and applied data science. The emphasis is on reproducible workflows, clear documentation, and decision-oriented outputs (tables, charts, and concise research notes).

## Contents
- [Featured Projects](#featured-projects)
- [Project Index](#project-index)
- [Repository Structure](#repository-structure)
- [Getting Started](#getting-started)
- [Reproducibility and Data Policy](#reproducibility-and-data-policy)
- [Security](#security)
- [License](#license)
- [Citation](#citation)
- [Contact](#contact)

---

## Featured Projects

### 1) Investment News Email Bot
**Goal:** Automate a daily investment/economics brief from curated sources with configurable filters.  
**Tools:** Python (RSS/HTTP), environment-based configuration, email delivery.  
**Location:** `projects/01-investment-news-email-bot/`  
**Status:** TODO — add screenshots + sample output email.

### 2) Portfolio Analytics Toolkit
**Goal:** Compute core portfolio statistics (risk/return, drawdowns, correlations, tracking error, rolling metrics) and export report-ready tables.  
**Tools:** Python (pandas/numpy/matplotlib), unit-test scaffolding.  
**Location:** `projects/02-portfolio-analytics/`  
**Status:** TODO — add sample data + notebook walk-through.

---

## Project Index

| Project | Tags | Primary Tools | Link |
|---|---|---:|---|
| Investment News Email Bot | automation, text, workflows | Python | `projects/01-investment-news-email-bot/` |
| Portfolio Analytics Toolkit | portfolio, risk, performance | Python | `projects/02-portfolio-analytics/` |
| Research Notes | memos, writing, methodology | Markdown/PDF | `projects/05-research-notes/` |

---

## Contact
Author: Connor Thomaston, M.S.  
Website: https://www.connorthomaston.com/
LinkedIn: www.linkedin.com/in/connorthomaston  
Email: connect@connorthomaston.com

---

## Repository Structure

```text
assets/                         # repo-level images (profile, screenshots, diagrams)
projects/                       # each project is self-contained with its own README
templates/                      # README/report/data-dictionary templates
docs/                           # portfolio and reproducibility notes

